...
import { storageGet, StorageSync, type StorageSyncValues } from "/dist/modules/storage.mjs";
...
import type { ArrayAccessor, ArrayMutator, ArrayObservable } from "/dist/modules/common.mjs";
...
import { ArrayBox, compatibility } from "/dist/modules/common.mjs";
...
export const handleMessage = await (async () => {
	...
	const applyConfig = async (config: Partial<StorageSyncValues>) => {
		...
		if (config.highlightMethod && controlsInfo.pageModificationAllowed) {
			highlighter.removeEngine();
			if (config.highlightMethod.paintUseExperimental !== undefined) {
				highlighter.signalPaintEngineMethod(
					config.highlightMethod.paintUseExperimental
						? (compatibility.browser === "firefox" ? "element" : "paint")
						: "url"
				);
			}
			if (config.highlightMethod.paintReplaceByClassic !== undefined) {
				//queuingPromise = highlighter.setEngine(config.highlighter.engine);
				//await queuingPromise;
				//queuingPromise = undefined;
				await highlighter.setEngine(config.highlightMethod.paintReplaceByClassic ? "ELEMENT" : "PAINT");
			}
		...
		if (config.highlightMethod?.hues) {
			hues = [ ...config.highlightMethod.hues ];
		}
	...
	await applyConfig(await storageGet("sync", [
