...
namespace Page {
	export interface InteractionInfo {
		...
		object?: {
			...
			columns: Array<Page.Interaction.ObjectColumnInfo>
		}
		textbox?: {
			className: string
			list?: {
		...
		submitters?: Array<Page.Interaction.SubmitterInfo>
		input?: Page.Interaction.InputInfo
		note?: {
			text?: string
	...
	export namespace Interaction {
		...
		export type InputFetch = () => InputType
...
export const getOrderedShortcut = (keys: Array<string>): Array<string> => {
	keys = keys.slice();
	keys.sort((a, b) => (
...
export const forInput = (input: HTMLInputElement, getText: (() => Promise<string | undefined>) | undefined,
...
/**/`
	...
	const shiftTabFromTab = (tab: HTMLElement, toRight: boolean, cycle: boolean) => {
		const tabNext = (() => {
			const tabNext = tab[toRight ? "nextElementSibling" : "previousElementSibling"] as HTMLElement | null;
			if (tabNext) {
				return tabNext;
			} if (cycle) {
	...
	const createSection = (() => {
		...
		const insertInput = (container: HTMLElement, inputInfo: Page.Interaction.InputInfo | undefined, id = "",
		...
		const insertNote = async (container: HTMLElement, noteInfo: Page.InteractionInfo["note"]) => {
			if (!noteInfo) {
				return;
			}
	...
	const createBrand = () => {
		...
		version.textContent = Manifest.getVersion();
	...
	return (panelsInfo: Array<Page.PanelInfo>, info: {
		titleText: string
		tabsFill: boolean
		borderShow: boolean
		brandShow: boolean
		...
		title.text = `${info.titleText} - ${Manifest.getName()}`;
