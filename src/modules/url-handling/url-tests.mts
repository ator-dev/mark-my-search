 ...
 */
const isUrlPageModificationAllowed = (urlString: string, urlFilter: StorageSyncValues["urlFilters"]["noPageModify"]): boolean => {
	try {
 ...
 */
const isUrlAutoFindAllowed = (urlString: string, urlFilter: StorageSyncValues["urlFilters"]["nonSearch"]): boolean => {
	try {
		return !isUrlFilteredIn(new URL(urlString), urlFilter);
	} catch {
